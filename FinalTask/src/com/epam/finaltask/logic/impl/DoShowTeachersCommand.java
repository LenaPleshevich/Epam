 ...
 */
public final class DoShowTeachersCommand implements ICommand {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws CommandException {
		...
		try {
			...
			List<User> teachers = dbDao.getTeacher(pageNumber);
			int numberOfUsers = dbDao.getNumberOfTeachers();
			...
			page = JspPageName.TEACHERS_PAGE;
			request.setAttribute(RequestParameterName.ID_COURSE, idCourse);
		} catch (DBDaoException e){
			logger.error("DBDaoException is thrown when trying to show teachers", e);
			throw new CommandException("DBDaoException is thrown when trying to show teachers", e);
		}
		return page;
	}
